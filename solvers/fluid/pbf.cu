...
namespace xayah::fluid::pbf::cuda_kernel {
    ...
    namespace {
        ...
        __device__ Float3 load_boundary_position(const sph::cuda_kernel::Boundary boundary, const std::uint32_t index) {
            return {
        ...
        __device__ Float3 poly6_gradient(const Float3 displacement, const float support_radius) {
            ...
            const float difference = squared_radius - squared_distance;
            const float coefficient = -945.0F / (32.0F * pi * powf(support_radius, 9.0F));
        ...
        __global__ void predict_forward_kernel(const std::uint32_t particle_count, const float time_step, const Float3 gravity, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::ConstVector controls, const sph::cuda_kernel::Vector predicted_positions) {
        ...
        __global__ void predict_jvp_kernel(const std::uint32_t particle_count, const float time_step, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ConstVector velocity_tangent, const sph::cuda_kernel::ConstVector control_tangent, const sph::cuda_kernel::Vector predicted_position_tangent) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void predict_vjp_kernel(const std::uint32_t particle_count, const float time_step, const sph::cuda_kernel::ConstVectorAdjoint predicted_position_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint velocity_adjoint, const sph::cuda_kernel::VectorAdjoint control_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void lambda_forward_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* densities, const float* relaxation, const sph::cuda_kernel::Vector gradient_sums, float* denominators, float* lambdas) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
                ...
                for (int offset_y = -1; offset_y <= 1; ++offset_y)
                    for (int offset_z = -1; offset_z <= 1; ++offset_z) {
                        ...
                        for (std::uint32_t sorted = range.boundary_first; sorted < range.boundary_last; ++sorted) {
                            ...
                            self_gradient = add(self_gradient, scale(spiky_gradient(subtract(position, load_boundary_position(boundary, neighbor)), support_radius), boundary.volumes[neighbor]));
                        }
                    }
        ...
        __global__ void lambda_jvp_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::ParticleParameterTangent particle_tangent, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* densities, const float* density_tangent, const float* relaxation, const float* relaxation_tangent, const sph::cuda_kernel::Vector gradient_sum_tangent, float* denominator_tangent, float* lambda_tangent) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void lambda_vjp_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* densities, const sph::cuda_kernel::ConstVector gradient_sums, const float* denominators, const double* lambda_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, double* density_adjoint, const sph::cuda_kernel::ParticleParameterAdjoint particle_adjoint, double* relaxation_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void correction_forward_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* lambdas, const float* strength, const float* exponent, const float* radius, const sph::cuda_kernel::Vector corrections) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void correction_jvp_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::ParticleParameterTangent particle_tangent, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* lambdas, const float* lambda_tangent, const float* strength, const float* strength_tangent, const float* exponent, const float* exponent_tangent, const float* radius, const float* radius_tangent, const sph::cuda_kernel::Vector correction_tangent) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void correction_vjp_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* lambdas, const float* strength, const float* exponent, const float* radius, const sph::cuda_kernel::ConstVectorAdjoint correction_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, double* lambda_adjoint, const sph::cuda_kernel::ParticleParameterAdjoint particle_adjoint, double* strength_adjoint, double* exponent_adjoint, double* radius_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void project_forward_kernel(const std::uint32_t particle_count, const sph::cuda_kernel::Domain domain, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector corrections, std::uint32_t* collision_masks, const sph::cuda_kernel::Vector projected_positions) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void project_jvp_kernel(const std::uint32_t particle_count, const std::uint32_t* collision_masks, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ConstVector correction_tangent, const sph::cuda_kernel::Vector projected_position_tangent) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void project_vjp_kernel(const std::uint32_t particle_count, const std::uint32_t* collision_masks, const sph::cuda_kernel::ConstVectorAdjoint projected_position_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint correction_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void reconstruct_forward_kernel(const std::uint32_t particle_count, const float inverse_time_step, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector corrected_positions, const sph::cuda_kernel::Vector velocities) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            store(velocities, particle, scale(subtract(load(corrected_positions, particle), load(positions, particle)), inverse_time_step));
        }

        __global__ void reconstruct_jvp_kernel(const std::uint32_t particle_count, const float inverse_time_step, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ConstVector corrected_position_tangent, const sph::cuda_kernel::Vector velocity_tangent) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void reconstruct_vjp_kernel(const std::uint32_t particle_count, const float inverse_time_step, const sph::cuda_kernel::ConstVectorAdjoint velocity_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint corrected_position_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void vorticity_forward_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Vector vorticities) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void vorticity_vjp_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::ConstVectorAdjoint vorticity_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint velocity_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void magnitude_forward_kernel(const std::uint32_t particle_count, const sph::cuda_kernel::ConstVector vorticities, float* magnitudes) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            magnitudes[particle] = length(load(vorticities, particle));
        }

        __global__ void normal_forward_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::Neighborhood neighborhood, const float* magnitudes, const sph::cuda_kernel::Vector normals, float* normalizers) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void magnitude_jvp_kernel(const std::uint32_t particle_count, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::ConstVector vorticity_tangent, const float* magnitudes, float* magnitude_tangent) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void normal_jvp_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::Neighborhood neighborhood, const float* magnitudes, const float* magnitude_tangent, const sph::cuda_kernel::ConstVector normals, const float* normalizers, const sph::cuda_kernel::Vector normal_tangent) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void normal_vjp_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::Neighborhood neighborhood, const float* magnitudes, const sph::cuda_kernel::ConstVector normals, const float* normalizers, const sph::cuda_kernel::ConstVectorAdjoint normal_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint vorticity_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void confinement_forward_kernel(const std::uint32_t particle_count, const float time_step, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::ConstVector normals, const float* confinement, const sph::cuda_kernel::Vector confined_velocities) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void confinement_jvp_kernel(const std::uint32_t particle_count, const float time_step, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::ConstVector normals, const float* confinement, const sph::cuda_kernel::ConstVector velocity_tangent, const sph::cuda_kernel::ConstVector vorticity_tangent, const sph::cuda_kernel::ConstVector normal_tangent, const float* confinement_tangent, const sph::cuda_kernel::Vector confined_velocity_tangent) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void confinement_vjp_kernel(const std::uint32_t particle_count, const float time_step, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::ConstVector normals, const float* confinement, const sph::cuda_kernel::ConstVectorAdjoint confined_velocity_adjoint, const sph::cuda_kernel::VectorAdjoint velocity_adjoint, const sph::cuda_kernel::VectorAdjoint vorticity_adjoint, const sph::cuda_kernel::VectorAdjoint normal_adjoint, double* confinement_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void xsph_forward_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::Neighborhood neighborhood, const float* viscosity, const sph::cuda_kernel::Vector output_velocities) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void xsph_jvp_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ConstVector velocity_tangent, const sph::cuda_kernel::Neighborhood neighborhood, const float* viscosity, const float* viscosity_tangent, const sph::cuda_kernel::Vector output_velocity_tangent) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
        ...
        __global__ void xsph_vjp_kernel(const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::Neighborhood neighborhood, const float* viscosity, const sph::cuda_kernel::ConstVectorAdjoint output_velocity_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint velocity_adjoint, double* viscosity_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
            const Float3 topology_position = load(topology_positions, particle);
            const Float3 position = load(positions, particle);
    ...
    void launch_predict_forward(void* stream, const std::uint32_t particle_count, const float time_step, const float gravity_x, const float gravity_y, const float gravity_z, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::ConstVector controls, const sph::cuda_kernel::Vector predicted_positions) {
        predict_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, time_step, {gravity_x, gravity_y, gravity_z}, positions, velocities, controls, predicted_positions);
        check_launch("launch_predict_forward");
    }

    void launch_predict_jvp(void* stream, const std::uint32_t particle_count, const float time_step, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ConstVector velocity_tangent, const sph::cuda_kernel::ConstVector control_tangent, const sph::cuda_kernel::Vector predicted_position_tangent) {
        predict_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, time_step, position_tangent, velocity_tangent, control_tangent, predicted_position_tangent);
        check_launch("launch_predict_jvp");
    }

    void launch_predict_vjp(void* stream, const std::uint32_t particle_count, const float time_step, const sph::cuda_kernel::ConstVectorAdjoint predicted_position_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint velocity_adjoint, const sph::cuda_kernel::VectorAdjoint control_adjoint) {
        predict_vjp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, time_step, predicted_position_adjoint, position_adjoint, velocity_adjoint, control_adjoint);
        check_launch("launch_predict_vjp");
    }

    void launch_lambda_forward(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* densities, const float* relaxation, const sph::cuda_kernel::Vector gradient_sums, float* denominators, float* lambdas) {
        lambda_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, particles, neighborhood, boundary, densities, relaxation, gradient_sums, denominators, lambdas);
        check_launch("launch_lambda_forward");
    }

    void launch_lambda_jvp(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::ParticleParameterTangent particle_tangent, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* densities, const float* density_tangent, const float* relaxation, const float* relaxation_tangent, const sph::cuda_kernel::Vector gradient_sum_tangent, float* denominator_tangent, float* lambda_tangent) {
        lambda_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, position_tangent, particles, particle_tangent, neighborhood, boundary, densities, density_tangent, relaxation, relaxation_tangent, gradient_sum_tangent, denominator_tangent, lambda_tangent);
        check_launch("launch_lambda_jvp");
    }

    void launch_lambda_vjp(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* densities, const sph::cuda_kernel::ConstVector gradient_sums, const float* denominators, const double* lambda_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, double* density_adjoint, const sph::cuda_kernel::ParticleParameterAdjoint particle_adjoint, double* relaxation_adjoint) {
        lambda_vjp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, particles, neighborhood, boundary, densities, gradient_sums, denominators, lambda_adjoint, position_adjoint, density_adjoint, particle_adjoint, relaxation_adjoint);
        check_launch("launch_lambda_vjp");
    }

    void launch_correction_forward(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* lambdas, const float* artificial_pressure_strength, const float* artificial_pressure_exponent, const float* artificial_pressure_radius, const sph::cuda_kernel::Vector corrections) {
        correction_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, particles, neighborhood, boundary, lambdas, artificial_pressure_strength, artificial_pressure_exponent, artificial_pressure_radius, corrections);
        check_launch("launch_correction_forward");
    }

    void launch_correction_jvp(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::ParticleParameterTangent particle_tangent, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* lambdas, const float* lambda_tangent, const float* artificial_pressure_strength, const float* artificial_pressure_strength_tangent, const float* artificial_pressure_exponent, const float* artificial_pressure_exponent_tangent, const float* artificial_pressure_radius, const float* artificial_pressure_radius_tangent, const sph::cuda_kernel::Vector correction_tangent) {
        correction_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, position_tangent, particles, particle_tangent, neighborhood, boundary, lambdas, lambda_tangent, artificial_pressure_strength, artificial_pressure_strength_tangent, artificial_pressure_exponent, artificial_pressure_exponent_tangent, artificial_pressure_radius, artificial_pressure_radius_tangent, correction_tangent);
        check_launch("launch_correction_jvp");
    }

    void launch_correction_vjp(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Boundary boundary, const float* lambdas, const float* artificial_pressure_strength, const float* artificial_pressure_exponent, const float* artificial_pressure_radius, const sph::cuda_kernel::ConstVectorAdjoint correction_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, double* lambda_adjoint, const sph::cuda_kernel::ParticleParameterAdjoint particle_adjoint, double* artificial_pressure_strength_adjoint, double* artificial_pressure_exponent_adjoint, double* artificial_pressure_radius_adjoint) {
        correction_vjp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, particles, neighborhood, boundary, lambdas, artificial_pressure_strength, artificial_pressure_exponent, artificial_pressure_radius, correction_adjoint, position_adjoint, lambda_adjoint, particle_adjoint, artificial_pressure_strength_adjoint, artificial_pressure_exponent_adjoint, artificial_pressure_radius_adjoint);
        check_launch("launch_correction_vjp");
    }

    void launch_project_forward(void* stream, const std::uint32_t particle_count, const sph::cuda_kernel::Domain domain, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector corrections, std::uint32_t* collision_masks, const sph::cuda_kernel::Vector projected_positions) {
        project_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, domain, positions, corrections, collision_masks, projected_positions);
        check_launch("launch_project_forward");
    }

    void launch_project_jvp(void* stream, const std::uint32_t particle_count, const std::uint32_t* collision_masks, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ConstVector correction_tangent, const sph::cuda_kernel::Vector projected_position_tangent) {
        project_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, collision_masks, position_tangent, correction_tangent, projected_position_tangent);
        check_launch("launch_project_jvp");
    }

    void launch_project_vjp(void* stream, const std::uint32_t particle_count, const std::uint32_t* collision_masks, const sph::cuda_kernel::ConstVectorAdjoint projected_position_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint correction_adjoint) {
        project_vjp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, collision_masks, projected_position_adjoint, position_adjoint, correction_adjoint);
        check_launch("launch_project_vjp");
    }

    void launch_reconstruct_forward(void* stream, const std::uint32_t particle_count, const float inverse_time_step, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector corrected_positions, const sph::cuda_kernel::Vector velocities) {
        reconstruct_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, inverse_time_step, positions, corrected_positions, velocities);
        check_launch("launch_reconstruct_forward");
    }

    void launch_reconstruct_jvp(void* stream, const std::uint32_t particle_count, const float inverse_time_step, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ConstVector corrected_position_tangent, const sph::cuda_kernel::Vector velocity_tangent) {
        reconstruct_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, inverse_time_step, position_tangent, corrected_position_tangent, velocity_tangent);
        check_launch("launch_reconstruct_jvp");
    }

    void launch_reconstruct_vjp(void* stream, const std::uint32_t particle_count, const float inverse_time_step, const sph::cuda_kernel::ConstVectorAdjoint velocity_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint corrected_position_adjoint) {
        reconstruct_vjp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, inverse_time_step, velocity_adjoint, position_adjoint, corrected_position_adjoint);
        check_launch("launch_reconstruct_vjp");
    }

    void launch_vorticity_forward(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Vector vorticities) {
        vorticity_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, velocities, neighborhood, vorticities);
        check_launch("launch_vorticity_forward");
    }

    void launch_vorticity_jvp(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ConstVector velocity_tangent, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::Vector vorticity_tangent) {
        vorticity_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, velocities, position_tangent, velocity_tangent, neighborhood, vorticity_tangent);
        check_launch("launch_vorticity_jvp");
    }

    void launch_vorticity_vjp(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::Neighborhood neighborhood, const sph::cuda_kernel::ConstVectorAdjoint vorticity_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint velocity_adjoint) {
        vorticity_vjp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, velocities, neighborhood, vorticity_adjoint, position_adjoint, velocity_adjoint);
        check_launch("launch_vorticity_vjp");
    }

    void launch_normal_forward(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::Neighborhood neighborhood, float* magnitudes, const sph::cuda_kernel::Vector normals, float* normalizers) {
        magnitude_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, vorticities, magnitudes);
        normal_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, neighborhood, magnitudes, normals, normalizers);
        check_launch("launch_normal_forward");
    }

    void launch_normal_jvp(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ConstVector vorticity_tangent, const sph::cuda_kernel::Neighborhood neighborhood, const float* magnitudes, const sph::cuda_kernel::ConstVector normals, const float* normalizers, float* magnitude_tangent, const sph::cuda_kernel::Vector normal_tangent) {
        magnitude_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, vorticities, vorticity_tangent, magnitudes, magnitude_tangent);
        normal_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, position_tangent, neighborhood, magnitudes, magnitude_tangent, normals, normalizers, normal_tangent);
        check_launch("launch_normal_jvp");
    }

    void launch_normal_vjp(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::Neighborhood neighborhood, const float* magnitudes, const sph::cuda_kernel::ConstVector normals, const float* normalizers, const sph::cuda_kernel::ConstVectorAdjoint normal_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint vorticity_adjoint) {
        normal_vjp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, vorticities, neighborhood, magnitudes, normals, normalizers, normal_adjoint, position_adjoint, vorticity_adjoint);
        check_launch("launch_normal_vjp");
    }

    void launch_confinement_forward(void* stream, const std::uint32_t particle_count, const float time_step, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::ConstVector normals, const float* confinement, const sph::cuda_kernel::Vector confined_velocities) {
        confinement_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, time_step, velocities, vorticities, normals, confinement, confined_velocities);
        check_launch("launch_confinement_forward");
    }

    void launch_confinement_jvp(void* stream, const std::uint32_t particle_count, const float time_step, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::ConstVector normals, const float* confinement, const sph::cuda_kernel::ConstVector velocity_tangent, const sph::cuda_kernel::ConstVector vorticity_tangent, const sph::cuda_kernel::ConstVector normal_tangent, const float* confinement_tangent, const sph::cuda_kernel::Vector confined_velocity_tangent) {
        confinement_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, time_step, vorticities, normals, confinement, velocity_tangent, vorticity_tangent, normal_tangent, confinement_tangent, confined_velocity_tangent);
        check_launch("launch_confinement_jvp");
    }

    void launch_confinement_vjp(void* stream, const std::uint32_t particle_count, const float time_step, const sph::cuda_kernel::ConstVector vorticities, const sph::cuda_kernel::ConstVector normals, const float* confinement, const sph::cuda_kernel::ConstVectorAdjoint confined_velocity_adjoint, const sph::cuda_kernel::VectorAdjoint velocity_adjoint, const sph::cuda_kernel::VectorAdjoint vorticity_adjoint, const sph::cuda_kernel::VectorAdjoint normal_adjoint, double* confinement_adjoint) {
        confinement_vjp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, time_step, vorticities, normals, confinement, confined_velocity_adjoint, velocity_adjoint, vorticity_adjoint, normal_adjoint, confinement_adjoint);
        check_launch("launch_confinement_vjp");
    }

    void launch_xsph_forward(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::Neighborhood neighborhood, const float* viscosity, const sph::cuda_kernel::Vector output_velocities) {
        xsph_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, velocities, neighborhood, viscosity, output_velocities);
        check_launch("launch_xsph_forward");
    }

    void launch_xsph_jvp(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::ConstVector position_tangent, const sph::cuda_kernel::ConstVector velocity_tangent, const sph::cuda_kernel::Neighborhood neighborhood, const float* viscosity, const float* viscosity_tangent, const sph::cuda_kernel::Vector output_velocity_tangent) {
        xsph_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, velocities, position_tangent, velocity_tangent, neighborhood, viscosity, viscosity_tangent, output_velocity_tangent);
        check_launch("launch_xsph_jvp");
    }

    void launch_xsph_vjp(void* stream, const std::uint32_t particle_count, const float support_radius, const sph::cuda_kernel::ConstVector topology_positions, const sph::cuda_kernel::ConstVector positions, const sph::cuda_kernel::ConstVector velocities, const sph::cuda_kernel::Neighborhood neighborhood, const float* viscosity, const sph::cuda_kernel::ConstVectorAdjoint output_velocity_adjoint, const sph::cuda_kernel::VectorAdjoint position_adjoint, const sph::cuda_kernel::VectorAdjoint velocity_adjoint, double* viscosity_adjoint) {
        xsph_vjp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, support_radius, topology_positions, positions, velocities, neighborhood, viscosity, output_velocity_adjoint, position_adjoint, velocity_adjoint, viscosity_adjoint);
        check_launch("launch_xsph_vjp");
    }
