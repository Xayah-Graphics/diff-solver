#ifndef XAYAH_FLUID_IISPH_H
#define XAYAH_FLUID_IISPH_H
...
namespace xayah::fluid::iisph::cuda_kernel {
    ...
    void launch_jacobi_update_forward(void* stream, std::uint32_t particle_count, float time_step, float reference_gradient_norm, sph::cuda_kernel::ParticleParameters particles, const float* densities, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, float* pressures);
    void launch_jacobi_update_jvp(void* stream, std::uint32_t particle_count, float time_step, float reference_gradient_norm, sph::cuda_kernel::ParticleParameters particles, sph::cuda_kernel::ParticleParameterTangent particle_tangent, const float* densities, const float* density_tangent, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, const float* previous_pressure_tangent, const float* predicted_density_tangent, const float* jacobi_relaxation_tangent, float* pressure_tangent);
    void launch_jacobi_update_vjp(void* stream, std::uint32_t particle_count, float time_step, float reference_gradient_norm, sph::cuda_kernel::ParticleParameters particles, const float* densities, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, const double* pressure_adjoint, sph::cuda_kernel::ParticleParameterAdjoint particle_adjoint, double* density_adjoint, double* previous_pressure_adjoint, double* predicted_density_adjoint, double* jacobi_relaxation_adjoint);
