#ifndef XAYAH_FLUID_DFSPH_H
#define XAYAH_FLUID_DFSPH_H
...
namespace xayah::fluid::dfsph::cuda_kernel {
    ...
    void launch_projection_update_forward(void* stream, std::uint32_t particle_count, float time_step, float reference_gradient_norm, sph::cuda_kernel::ParticleParameters particles, const float* densities, const float* target_densities, const float* previous_pressures, const float* predicted_densities, const float* projection_relaxation, float* pressures);
    void launch_projection_update_jvp(void* stream, std::uint32_t particle_count, float time_step, float reference_gradient_norm, sph::cuda_kernel::ParticleParameters particles, sph::cuda_kernel::ParticleParameterTangent particle_tangent, const float* densities, const float* density_tangent, const float* target_densities, const float* target_density_tangent, const float* previous_pressures, const float* predicted_densities, const float* projection_relaxation, const float* previous_pressure_tangent, const float* predicted_density_tangent, const float* projection_relaxation_tangent, float* pressure_tangent);
    void launch_projection_update_vjp(void* stream, std::uint32_t particle_count, float time_step, float reference_gradient_norm, sph::cuda_kernel::ParticleParameters particles, const float* densities, const float* target_densities, const float* previous_pressures, const float* predicted_densities, const float* projection_relaxation, const double* pressure_adjoint, sph::cuda_kernel::ParticleParameterAdjoint particle_adjoint, double* density_adjoint, double* target_density_adjoint, double* previous_pressure_adjoint, double* predicted_density_adjoint, double* projection_relaxation_adjoint);
