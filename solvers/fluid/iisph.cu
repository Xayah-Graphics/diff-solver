...
namespace xayah::fluid::iisph::cuda_kernel {
    ...
    namespace {
        ...
        __global__ void jacobi_update_forward_kernel(const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const sph::cuda_kernel::ParticleParameters particles, const float* densities, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, float* pressures) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void jacobi_update_jvp_kernel(const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::ParticleParameterTangent particle_tangent, const float* densities, const float* density_tangent, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, const float* previous_pressure_tangent, const float* predicted_density_tangent, const float* jacobi_relaxation_tangent, float* pressure_tangent) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
        ...
        __global__ void jacobi_update_vjp_kernel(const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const sph::cuda_kernel::ParticleParameters particles, const float* densities, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, const double* pressure_adjoint, const sph::cuda_kernel::ParticleParameterAdjoint particle_adjoint, double* density_adjoint, double* previous_pressure_adjoint, double* predicted_density_adjoint, double* jacobi_relaxation_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
    ...
    void launch_jacobi_update_forward(void* stream, const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const sph::cuda_kernel::ParticleParameters particles, const float* densities, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, float* pressures) {
        jacobi_update_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, time_step, reference_gradient_norm, particles, densities, previous_pressures, predicted_densities, jacobi_relaxation, pressures);
        check_launch("launch_jacobi_update_forward");
    }

    void launch_jacobi_update_jvp(void* stream, const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::ParticleParameterTangent particle_tangent, const float* densities, const float* density_tangent, const float* previous_pressures, const float* predicted_densities, const float* jacobi_relaxation, const float* previous_pressure_tangent, const float* predicted_density_tangent, const float* jacobi_relaxation_tangent, float* pressure_tangent) {
