...
namespace xayah::fluid::pcisph {
    ...
    namespace {
        ...
        void zero_parameters(cuda::Resource& resource, ParameterTangent& parameters) {
            ...
            resource.zero(parameters.pressure_relaxation.data, parameters.pressure_relaxation.size * sizeof(float));
        }
        ...
        void zero_parameters(cuda::Resource& resource, ParameterAdjoint& parameters) {
            ...
            resource.zero(parameters.pressure_relaxation.data, parameters.pressure_relaxation.size * sizeof(double));
        }
    ...
    Parameters Model::allocate_parameters(ExecutionContext& context) const {
        return {
            ...
            .pressure_relaxation = cuda::Buffer<float>(context.resource, configuration.fluid.particle_count),
        };
    }
    ...
    ParameterAdjoint Model::allocate_parameter_adjoint(ExecutionContext& context) const {
        ParameterAdjoint adjoint{
            ...
            .pressure_relaxation = cuda::Buffer<double>(context.resource, configuration.fluid.particle_count),
        };
    ...
    void Model::forward_step(const State& state, const Control& control, const Parameters& parameters, State& next_state, StepCache& step_cache, ExecutionContext& context) const {
        ...
        for (std::uint32_t iteration = 1u; iteration <= configuration.pressure_iterations; ++iteration) {
            ...
            cuda_kernel::launch_pressure_update_forward(context.resource->native_stream, configuration.fluid.particle_count, configuration.fluid.time_step, reference_gradient_norm_, kernel_parameters(parameters.particles), context.primal_scratch_.pressures.values.data, context.primal_scratch_.predicted_densities.values.data, parameters.pressure_relaxation.data, context.primal_scratch_.pressures.values.data);
    ...
    void Model::jvp_step(const State& state, const Control&, const Parameters& parameters, const State&, const StepCache& step_cache, const StateTangent& state_tangent, const ControlTangent& control_tangent, const ParameterTangent& parameter_tangent, StateTangent& next_state_tangent, ExecutionContext& context) const {
        ...
        for (std::uint32_t iteration = 1u; iteration <= configuration.pressure_iterations; ++iteration) {
            ...
            cuda_kernel::launch_pressure_update_jvp(context.resource->native_stream, configuration.fluid.particle_count, configuration.fluid.time_step, reference_gradient_norm_, kernel_parameters(parameters.particles), kernel_parameters(parameter_tangent.particles), context.primal_scratch_.pressures.values.data, context.primal_scratch_.predicted_densities.values.data, parameters.pressure_relaxation.data, context.tangent_scratch_.pressures.values.data, context.tangent_scratch_.predicted_densities.values.data, parameter_tangent.pressure_relaxation.data, context.tangent_scratch_.pressures.values.data);
            cuda_kernel::launch_pressure_update_forward(context.resource->native_stream, configuration.fluid.particle_count, configuration.fluid.time_step, reference_gradient_norm_, kernel_parameters(parameters.particles), context.primal_scratch_.pressures.values.data, context.primal_scratch_.predicted_densities.values.data, parameters.pressure_relaxation.data, context.primal_scratch_.pressures.values.data);
    ...
    void Model::vjp_step(const State& state, const Control&, const Parameters& parameters, const State&, const StepCache& step_cache, const StateAdjoint& next_state_adjoint, StateAdjoint& previous_state_adjoint, ControlAdjoint& control_adjoint, ParameterAdjoint& parameter_adjoint, ExecutionContext& context) const {
        ...
        for (std::size_t checkpoint = step_cache.checkpoints.size() - 1u; checkpoint > 0u; --checkpoint) {
            ...
            for (std::uint32_t iteration = first.iteration + 1u; iteration <= last.iteration; ++iteration) {
                ...
                cuda_kernel::launch_pressure_update_forward(context.resource->native_stream, configuration.fluid.particle_count, configuration.fluid.time_step, reference_gradient_norm_, kernel_parameters(parameters.particles), previous.pressures.values.data, current.predicted_densities.values.data, parameters.pressure_relaxation.data, current.pressures.values.data);
            ...
            for (std::uint32_t iteration = last.iteration; iteration > first.iteration; --iteration) {
                ...
                cuda_kernel::launch_pressure_update_vjp(context.resource->native_stream, configuration.fluid.particle_count, configuration.fluid.time_step, reference_gradient_norm_, kernel_parameters(parameters.particles), previous.pressures.values.data, current.predicted_densities.values.data, parameters.pressure_relaxation.data, context.adjoint_scratch_.pressures.values.data, kernel_parameters(parameter_adjoint.particles), context.previous_adjoint_scratch_.pressures.values.data, context.adjoint_scratch_.predicted_densities.values.data, parameter_adjoint.pressure_relaxation.data);
