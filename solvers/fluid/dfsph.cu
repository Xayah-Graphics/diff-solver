...
namespace xayah::fluid::dfsph::cuda_kernel {
    ...
    namespace {
        ...
        __global__ void projection_update_vjp_kernel(const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const sph::cuda_kernel::ParticleParameters particles, const float* densities, const float* target_densities, const float* previous_pressures, const float* predicted_densities, const float* projection_relaxation, const double* pressure_adjoint, const sph::cuda_kernel::ParticleParameterAdjoint particle_adjoint, double* density_adjoint, double* target_density_adjoint, double* previous_pressure_adjoint, double* predicted_density_adjoint, double* projection_relaxation_adjoint) {
            const std::uint32_t particle = blockIdx.x * blockDim.x + threadIdx.x;
            if (particle >= particle_count) return;
    ...
    void launch_projection_update_forward(void* stream, const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const sph::cuda_kernel::ParticleParameters particles, const float* densities, const float* target_densities, const float* previous_pressures, const float* predicted_densities, const float* projection_relaxation, float* pressures) {
        projection_update_forward_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, time_step, reference_gradient_norm, particles, densities, target_densities, previous_pressures, predicted_densities, projection_relaxation, pressures);
        check_launch("launch_projection_update_forward");
    }

    void launch_projection_update_jvp(void* stream, const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const sph::cuda_kernel::ParticleParameters particles, const sph::cuda_kernel::ParticleParameterTangent particle_tangent, const float* densities, const float* density_tangent, const float* target_densities, const float* target_density_tangent, const float* previous_pressures, const float* predicted_densities, const float* projection_relaxation, const float* previous_pressure_tangent, const float* predicted_density_tangent, const float* projection_relaxation_tangent, float* pressure_tangent) {
        projection_update_jvp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, time_step, reference_gradient_norm, particles, particle_tangent, densities, density_tangent, target_densities, target_density_tangent, previous_pressures, predicted_densities, projection_relaxation, previous_pressure_tangent, predicted_density_tangent, projection_relaxation_tangent, pressure_tangent);
        check_launch("launch_projection_update_jvp");
    }

    void launch_projection_update_vjp(void* stream, const std::uint32_t particle_count, const float time_step, const float reference_gradient_norm, const sph::cuda_kernel::ParticleParameters particles, const float* densities, const float* target_densities, const float* previous_pressures, const float* predicted_densities, const float* projection_relaxation, const double* pressure_adjoint, const sph::cuda_kernel::ParticleParameterAdjoint particle_adjoint, double* density_adjoint, double* target_density_adjoint, double* previous_pressure_adjoint, double* predicted_density_adjoint, double* projection_relaxation_adjoint) {
        projection_update_vjp_kernel<<<blocks(particle_count), block_size, 0, static_cast<cudaStream_t>(stream)>>>(particle_count, time_step, reference_gradient_norm, particles, densities, target_densities, previous_pressures, predicted_densities, projection_relaxation, pressure_adjoint, particle_adjoint, density_adjoint, target_density_adjoint, previous_pressure_adjoint, predicted_density_adjoint, projection_relaxation_adjoint);
        check_launch("launch_projection_update_vjp");
    }
