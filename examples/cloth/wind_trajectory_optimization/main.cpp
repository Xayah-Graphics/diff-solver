...
int main() {
    ...
    for (std::size_t iteration = 0u; iteration < optimization_iterations; ++iteration) {
        task.optimize_step();
        if (task.metrics.iteration % 10u == 0u) std::println("  iteration {:3}: loss={:.9e}, loss_ratio={:.3e}, keyframe_error={:.3e}, gradient_norm={:.9e}", task.metrics.iteration, task.metrics.loss, task.metrics.loss_ratio, task.metrics.keyframe_relative_error, task.metrics.gradient_norm);
    }
    ...
    for (std::size_t keyframe = 0; keyframe < task.target_keyframes.size(); ++keyframe) std::println("  keyframe {}: target=({:+.6f}, {:+.6f}), estimate=({:+.6f}, {:+.6f})", keyframe, task.target_keyframes[keyframe].x, task.target_keyframes[keyframe].z, task.estimated_keyframes[keyframe].x, task.estimated_keyframes[keyframe].z);
    std::cout.flush();
