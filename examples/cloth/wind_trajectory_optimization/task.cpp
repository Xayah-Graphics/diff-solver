...
namespace xayah::cloth::examples::wind_trajectory_optimization {
    ...
    WindTrajectoryOptimizationTask::WindTrajectoryOptimizationTask(Configuration configuration, WindTrajectoryOptimizationOptions next_options)
        : options(next_options), model(std::move(configuration)), context(model.allocate_context(ExecutionMode::differentiable)), target_trajectory{}, estimated_trajectory{},
          target_keyframes{
        ...
        upload(*context.resource, model.configuration.rest_positions, initial_state_.positions);
        upload(*context.resource, std::vector<Vector3>(model.configuration.rest_positions.size()), initial_state_.velocities);
        ...
        upload_parameters();
        {
            ...
            for (std::size_t step = 0; step < options.trajectory_steps; ++step) target_controls.push_back(model.allocate_control(context));
        ...
        for (std::size_t step = 0; step < target_trajectory.states.size(); ++step) trajectory_adjoint_.push_back(model.allocate_state_adjoint(context));
        reset();
    }
    ...
    WindTrajectoryGradientCheck WindTrajectoryOptimizationTask::check_gradient(const xayah::solver::TapeMode tape_mode, const float epsilon) {
        ...
        for (std::size_t step = 0; step < options.trajectory_steps; ++step) control_tangents.push_back(model.allocate_control_tangent(context));
        ...
        for (std::size_t step = 0; step < control_tangents.size(); ++step) task_cuda::launch_write_control(stream, particles, static_cast<std::uint32_t>(step), static_cast<std::uint32_t>(options.trajectory_steps), context.device_topology.anchor_mask.data, device_keyframes_, kernel_field(control_tangents[step].external_forces));
        ...
        for (std::size_t step = 1; step < trajectory_tangent.size(); ++step) task_cuda::launch_position_tangent_inner_product(stream, particles, kernel_field(static_cast<const VectorField&>(trajectory_adjoint_[step].positions)), kernel_field(trajectory_tangent[step].positions), scalar_);
        ...
        for (std::size_t keyframe = 0; keyframe < keyframe_count; ++keyframe) vjp_inner_product += keyframe_gradients_[2u * keyframe] * direction[keyframe].x + keyframe_gradients_[2u * keyframe + 1u] * direction[keyframe].z;
    ...
    void WindTrajectoryOptimizationTask::write_controls(std::vector<Control>& controls, const std::span<const Vector3> keyframes) {
        ...
        const cudaStream_t stream     = static_cast<cudaStream_t>(context.resource->native_stream);
        const std::uint32_t particles = static_cast<std::uint32_t>(model.configuration.rest_positions.size());
        for (std::size_t step = 0; step < controls.size(); ++step) task_cuda::launch_write_control(stream, particles, static_cast<std::uint32_t>(step), static_cast<std::uint32_t>(options.trajectory_steps), context.device_topology.anchor_mask.data, device_keyframes_, kernel_field(controls[step].external_forces));
    }
    ...
    double WindTrajectoryOptimizationTask::trajectory_loss(const xayah::solver::Trajectory<::xayah::cloth::State, StepCache>& trajectory) {
        ...
        resource.zero(scalar_, sizeof(double));
        for (std::size_t step = 1; step < trajectory.states.size(); ++step) task_cuda::launch_position_loss(stream, particles, normalization, kernel_field(trajectory.states[step].positions), kernel_field(static_cast<const VectorField&>(target_trajectory.states[step].positions)), scalar_);
        double loss;
    ...
    void WindTrajectoryOptimizationTask::evaluate(const xayah::solver::TapeMode tape_mode) {
        ...
        resource.zero(scalar_, sizeof(double));
        for (std::size_t step = 1; step < estimated_trajectory.states.size(); ++step) task_cuda::launch_position_loss_seed(stream, particles, normalization, kernel_field(static_cast<const VectorField&>(estimated_trajectory.states[step].positions)), kernel_field(static_cast<const VectorField&>(target_trajectory.states[step].positions)), kernel_field(trajectory_adjoint_[step].positions), scalar_);
        double loss;
        resource.copy_to_host(&loss, scalar_, sizeof(double));

        const auto gradients = xayah::solver::vjp(model, context, estimated_trajectory, std::span<const Control>{estimated_controls_}, parameters_, std::span<const StateAdjoint>{trajectory_adjoint_});
        resource.zero(device_keyframe_gradients_, variable_count * sizeof(double));
        for (std::size_t step = 0; step < gradients.controls.size(); ++step) task_cuda::launch_accumulate_keyframe_gradient(stream, particles, static_cast<std::uint32_t>(step), static_cast<std::uint32_t>(options.trajectory_steps), context.device_topology.anchor_mask.data, kernel_field(gradients.controls[step].external_forces), device_keyframe_gradients_);
