...
namespace xayah::cloth::examples::forward::simulation_cuda {
    ...
    void launch_write_control(
        ...
        Field external_forces,
        double* metrics);

    void launch_particle_metrics(
        cudaStream_t stream,
        std::uint32_t rows,
        std::uint32_t columns,
        const float* masses,
        ConstField positions,
        ConstField velocities,
        double* metrics);

    void launch_strain_metrics(
        cudaStream_t stream,
        ...
        ConstField positions,
        double* maximum_absolute_strain);
