...
namespace xayah::cloth::examples::stretch_stiffness_inverse {
    ...
    StretchStiffnessInverseTask::StretchStiffnessInverseTask(Configuration configuration, StretchStiffnessInverseOptions next_options) : options(next_options), model(std::move(configuration)), context(model.allocate_context(ExecutionMode::differentiable)), target_trajectory{}, estimated_trajectory{}, metrics{}, initial_state_(model.allocate_state(context)), controls_{}, estimated_parameters_(model.allocate_parameters(context)), trajectory_adjoint_{}, scalar_(static_cast<double*>(context.resource->allocate(sizeof(double)))), log_stiffness_{}, first_moment_{}, second_moment_{}, adam_step_{} {
        ...
        for (std::size_t step = 0; step < options.trajectory_steps; ++step) controls_.push_back(model.allocate_control(context));
        upload_parameters(estimated_parameters_, options.initial_stretch_stiffness);
        {
            Parameters target_parameters = model.allocate_parameters(context);
    ...
    StretchStiffnessGradientCheck StretchStiffnessInverseTask::check_gradient(const xayah::solver::TapeMode tape_mode, const float epsilon) {
        ...
        for (std::size_t step = 0; step < controls_.size(); ++step) control_tangents.push_back(model.allocate_control_tangent(context));
        ...
        for (std::size_t step = 1; step < trajectory_tangent.size(); ++step) inverse_cuda::launch_position_tangent_inner_product(stream, static_cast<std::uint32_t>(model.configuration.rest_positions.size()), kernel_field(static_cast<const VectorField&>(trajectory_adjoint_[step].positions)), kernel_field(trajectory_tangent[step].positions), scalar_);
    ...
    void StretchStiffnessInverseTask::upload_parameters(Parameters& parameters, const float stretch_stiffness) {
        ...
        upload(*context.resource, std::vector<float>(model.topology.stretch_springs.size(), stretch_stiffness), parameters.stretch_stiffnesses);
    ...
    void StretchStiffnessInverseTask::evaluate(const xayah::solver::TapeMode tape_mode) {
        ...
        resource.zero(scalar_, sizeof(double));
        for (std::size_t step = 1; step < estimated_trajectory.states.size(); ++step) inverse_cuda::launch_position_loss_seed(stream, particles, normalization, kernel_field(static_cast<const VectorField&>(estimated_trajectory.states[step].positions)), kernel_field(static_cast<const VectorField&>(target_trajectory.states[step].positions)), kernel_field(trajectory_adjoint_[step].positions), scalar_);
        double loss;
        resource.copy_to_host(&loss, scalar_, sizeof(double));
        resource.synchronize();

        const auto gradients = xayah::solver::vjp(model, context, estimated_trajectory, std::span<const Control>{controls_}, estimated_parameters_, std::span<const StateAdjoint>{trajectory_adjoint_});
        resource.zero(scalar_, sizeof(double));
    ...
    double StretchStiffnessInverseTask::loss_at_log_stiffness(const double value, const xayah::solver::TapeMode tape_mode) {
        Parameters parameters = model.allocate_parameters(context);
