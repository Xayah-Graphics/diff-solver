...
int main() {
    try {
        ...
        for (std::uint32_t step = 0u; step < 960u; ++step) {
            ...
            std::println("step {:3}  time {:.3f} s  density mass {:.6f} max {:.5f}  temperature max {:.5f}  velocity max {:.5f}  CFL {:.4f}  divergence {:.4e} -> {:.4e} ({:.4f})  step {:.3f} ms  average {:.3f} ms", metrics.step, metrics.physical_time, metrics.density_mass, metrics.density_maximum, metrics.temperature_maximum, metrics.maximum_velocity, metrics.cfl, metrics.pre_projection_divergence_rms, metrics.post_projection_divergence_rms, metrics.divergence_ratio, metrics.step_milliseconds, metrics.average_step_milliseconds);
        }
        return 0;
