...
int main() {
    xayah::smoke::examples::wind_trajectory_optimization::WindTrajectoryOptimizationTask task;
    const xayah::smoke::examples::wind_trajectory_optimization::WindTrajectoryGradientCheck store_all = task.check_gradient(xayah::solver::TapeMode::store_all, epsilon);
    const xayah::smoke::examples::wind_trajectory_optimization::WindTrajectoryGradientCheck recompute = task.check_gradient(xayah::solver::TapeMode::recompute_step_cache, epsilon);

    std::println("CUDA smoke wind-trajectory optimization gradient checks");
    for (const auto [name, metrics] : std::array{
             std::pair<std::string_view, xayah::smoke::examples::wind_trajectory_optimization::WindTrajectoryGradientCheck>{"store_all", store_all},
    ...
    std::println("\nCUDA smoke wind-trajectory optimization");
