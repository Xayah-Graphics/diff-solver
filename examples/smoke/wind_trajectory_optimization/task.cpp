...
namespace xayah::smoke::examples::wind_trajectory_optimization {
    ...
    WindTrajectoryOptimizationTask::WindTrajectoryOptimizationTask(WindTrajectoryOptimizationOptions next_options)
        : options(next_options), model(make_configuration()), context(model.allocate_context(ExecutionMode::differentiable)), target_trajectory{}, estimated_trajectory{},
          target_keyframes{
        ...
        for (std::size_t step = 0u; step < options.trajectory_steps; ++step) {
            estimated_controls_.push_back(model.allocate_control(context));
            probe_controls_.push_back(model.allocate_control(context));
        }
        upload_parameters();
        {
            std::vector<Control> target_controls;
            target_controls.reserve(options.trajectory_steps);
            for (std::size_t step = 0u; step < options.trajectory_steps; ++step) target_controls.push_back(model.allocate_control(context));
        ...
        trajectory_adjoint_.reserve(target_trajectory.states.size());
        for (std::size_t step = 0u; step < target_trajectory.states.size(); ++step) trajectory_adjoint_.push_back(model.allocate_state_adjoint(context));
        reset();
    }
    ...
    void WindTrajectoryOptimizationTask::optimize_step() {
        ...
        for (std::size_t variable = 0u; variable < variable_count; ++variable) {
            ...
            if (variable % 2u == 0u)
                estimated_keyframes[variable / 2u].x -= static_cast<float>(update);
            else
                estimated_keyframes[variable / 2u].z -= static_cast<float>(update);
        }
        ++metrics.iteration;
        evaluate(solver::TapeMode::recompute_step_cache);
    }
    ...
    WindTrajectoryGradientCheck WindTrajectoryOptimizationTask::check_gradient(const solver::TapeMode tape_mode, const float epsilon) {
        ...
        StateTangent initial_state_tangent = model.allocate_state_tangent(context);
        std::vector<ControlTangent> control_tangents;
        control_tangents.reserve(options.trajectory_steps);
        for (std::size_t step = 0u; step < options.trajectory_steps; ++step) control_tangents.push_back(model.allocate_control_tangent(context));
        write_control_tangents(control_tangents, direction);
        ParameterTangent parameter_tangent = model.allocate_parameter_tangent(context);
        ...
        const cudaStream_t stream      = static_cast<cudaStream_t>(context.resource->native_stream);
        const std::uint32_t cell_count = model.configuration.resolution[0] * model.configuration.resolution[1] * model.configuration.resolution[2];
        for (std::size_t step = 1u; step < trajectory_tangent.size(); ++step) task_cuda::launch_density_tangent_inner_product(stream, cell_count, trajectory_adjoint_[step].density.values.data, trajectory_tangent[step].density.values.data, scalar_);
        ...
        for (std::size_t keyframe = 0u; keyframe < keyframe_count; ++keyframe) vjp_inner_product += keyframe_gradients_[2u * keyframe] * direction[keyframe].x + keyframe_gradients_[2u * keyframe + 1u] * direction[keyframe].z;
    ...
    void WindTrajectoryOptimizationTask::upload_parameters() {
        constexpr float vorticity_confinement = 0.0F;
        ...
        context.resource->copy_from_host(parameters_.vorticity_confinement.data, &vorticity_confinement, sizeof(float));
        context.resource->synchronize();
    }
    ...
    void WindTrajectoryOptimizationTask::write_controls(std::vector<Control>& controls, const std::span<const Vector3> keyframes) {
        ...
        for (std::size_t step = 0u; step < controls.size(); ++step) task_cuda::launch_write_control(stream, grid, static_cast<std::uint32_t>(step), static_cast<std::uint32_t>(options.trajectory_steps), device_keyframes_, options.density_source_rate, options.temperature_source_rate, controls[step].density_source.values.data, controls[step].temperature_source.values.data, task_vector(controls[step].external_acceleration));
    }
    ...
    void WindTrajectoryOptimizationTask::write_control_tangents(std::vector<ControlTangent>& controls, const std::span<const Vector3> keyframes) {
        ...
        const cudaStream_t stream  = static_cast<cudaStream_t>(context.resource->native_stream);
        const task_cuda::Grid grid = task_grid(model.configuration);
        for (std::size_t step = 0u; step < controls.size(); ++step) task_cuda::launch_write_control_tangent(stream, grid, static_cast<std::uint32_t>(step), static_cast<std::uint32_t>(options.trajectory_steps), device_keyframes_, controls[step].density_source.values.data, controls[step].temperature_source.values.data, task_vector(controls[step].external_acceleration));
    }
    ...
    double WindTrajectoryOptimizationTask::trajectory_loss(const solver::Trajectory<State, StepCache>& trajectory) {
        ...
        const cudaStream_t stream      = static_cast<cudaStream_t>(context.resource->native_stream);
        const std::uint32_t cell_count = model.configuration.resolution[0] * model.configuration.resolution[1] * model.configuration.resolution[2];
        for (std::size_t step = 1u; step < trajectory.states.size(); ++step) task_cuda::launch_density_loss(stream, cell_count, inverse_target_frame_energies_[step - 1u], trajectory.states[step].density.values.data, target_trajectory.states[step].density.values.data, scalar_);
        double loss;
    ...
    void WindTrajectoryOptimizationTask::evaluate(const solver::TapeMode tape_mode) {
        ...
        const cudaStream_t stream      = static_cast<cudaStream_t>(context.resource->native_stream);
        const std::uint32_t cell_count = model.configuration.resolution[0] * model.configuration.resolution[1] * model.configuration.resolution[2];
        for (std::size_t step = 1u; step < estimated_trajectory.states.size(); ++step) task_cuda::launch_density_loss_seed(stream, cell_count, inverse_target_frame_energies_[step - 1u], estimated_trajectory.states[step].density.values.data, target_trajectory.states[step].density.values.data, trajectory_adjoint_[step].density.values.data, scalar_);
        double loss;
        context.resource->copy_to_host(&loss, scalar_, sizeof(double));

        const auto gradients = solver::vjp(model, context, estimated_trajectory, std::span<const Control>{estimated_controls_}, parameters_, std::span<const StateAdjoint>{trajectory_adjoint_});
        context.resource->zero(device_keyframe_gradients_, variable_count * sizeof(double));
        const task_cuda::Grid grid = task_grid(model.configuration);
        for (std::size_t step = 0u; step < gradients.controls.size(); ++step) task_cuda::launch_accumulate_keyframe_gradient(stream, grid, static_cast<std::uint32_t>(step), static_cast<std::uint32_t>(options.trajectory_steps), task_vector(static_cast<const CenteredVectorAdjointField&>(gradients.controls[step].external_acceleration)), device_keyframe_gradients_);
